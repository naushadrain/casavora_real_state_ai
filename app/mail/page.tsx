...
// Mailchimp's classic embed form. `MC_MERGE_FIELDS` backs the SMS-phone
// helper functions supplied by the external mc-sms-phone.js script.
const MC_EMBED_INIT_SCRIPT = `
...
function generateDropdownOptions(smsProgramData) {
  ...
  var programs = smsProgramData.filter(function(p, i, arr) {
...
function getDefaultPlaceholder(countryCode) {
  if (!countryCode || typeof countryCode !== 'string') {
    return MC_PHONE_PLACEHOLDERS[0].placeholder;
  }
  const selectedPlaceholder = MC_PHONE_PLACEHOLDERS.find(function(item) {
    return item && item.countryCode === countryCode;
  });
  return selectedPlaceholder ? selectedPlaceholder.placeholder : MC_PHONE_PLACEHOLDERS[0].placeholder;
}
...
function initializeSmsPhoneDropdown(fieldName) {
  ...
  var smsField = Object.values(MC_MERGE_FIELDS).find(function(f) { return f.name === fieldName && f.type === 'smsphone'; });
  ...
  displayFlag?.addEventListener('click', function() {
    dropdown.focus();
  });
  ...
  dropdown?.addEventListener('change', function() {
    ...
    var smsField2 = Object.values(MC_MERGE_FIELDS).find(function(f) { return f.name === fieldName && f.type === 'smsphone'; });
    var isRequired2 = smsField2 ? smsField2.required : false;
    var shouldAppendCountryCode2 = smsNotRequiredRemoveCountryCodeEnabled ? isRequired2 : true;

    var phoneInput2 = document.querySelector('#mce-' + fieldName);
    if (phoneInput2 && selectedProgram.countryCallingCode && shouldAppendCountryCode2) {
      phoneInput2.value = selectedProgram.countryCallingCode;
    }
...
// The DOM is already parsed by the time this script runs client-side, so
// initialize immediately instead of waiting on a DOMContentLoaded event
// that already fired.
...
export default function MailPage() {
  ...
  return (
    <div id="mc_embed_shell">
      ...
      <div id="mc_embed_signup" className={styles.embedShell}>
        <form
          action="https://gmail.us17.list-manage.com/subscribe/post?u=56e60c8047e6c7041c587489e&id=1f32ef8b3c&f_id=008fc3e1f0"
          method="post"
        ...
        >
          <div id="mc_embed_signup_scroll">
            ...
            <div aria-hidden="true" style={{ position: "absolute", left: "-5000px" }}>
              <input type="text" name="b_56e60c8047e6c7041c587489e_1f32ef8b3c" tabIndex={-1} />
            </div>
            <div className="clear">
      ...
      {/* Mailchimp's classic embed scripts must load in order: validate -> sms-phone -> inline init */}
      <Script
        id="mc-validate"
        src="//s3.amazonaws.com/downloads.mailchimp.com/js/mc-validate.js"
        strategy="afterInteractive"
        onLoad={() => setValidateLoaded(true)}
      />
      {validateLoaded && (
        <Script
          id="mc-sms-phone"
          src="//s3.amazonaws.com/downloads.mailchimp.com/js/mc-sms-phone.js"
          strategy="afterInteractive"
          onLoad={() => setSmsLoaded(true)}
        />
      )}
      {smsLoaded && (
        <Script id="mc-embed-init" strategy="afterInteractive" dangerouslySetInnerHTML={{ __html: MC_EMBED_INIT_SCRIPT }} />
      )}
    </div>
  );
}
